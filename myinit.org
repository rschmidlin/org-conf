...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package counsel
	...
	:config
	(ivy-mode 1)
	(define-key minibuffer-inactive-mode-map (kbd "<tab>") 'ivy-alt-done))
#+END_SRC
* Navigation
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  ...
  (global-set-key (kbd "M-I") 'scroll-down) ;; collides with minibuffer otherwise
  (global-set-key (kbd "M-K") 'scroll-up) ;; collides with minibuffer otherwise
  ...
  (eval-after-load "ace-window-mode" (define-key ergoemacs-light-mode-map (kbd "M-s") 'ace-window))
  ...
  (global-set-key (kbd "<f3>") 'dump-jump-back)
  (global-set-key (kbd "<f4>") 'dumb-jump-go)
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; (toggle-truncate-lines) -> apply to all buffers
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun raul-find-references ()
    ...
    (cond
     ...
     ((eq major-mode 'c++-mode) (counsel-gtags-dwim))
     ((eq major-mode 'c-mode) (counsel-gtags-dwim))
  ...
  (defun raul-navigate-forward ()
    ...
    (cond
     ((eq major-mode 'python-mode) (message "Not supported in Jedi"))
     ((eq major-mode 'c++-mode) (counsel-gtags-go-forward))
     ((eq major-mode 'c-mode) (counsel-gtags-go-forward))
     (t nil)))
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun recentf-save-list-cyclicly ()
    (interactive)
    (when (fboundp 'recentf-save-list)
      (recentf-save-list)))
  (run-at-time nil (* 5 60) 'recentf-save-list-cyclicly)
  (defun smex-save-to-file-cyclicly ()
    (interactive)
    (when (boundp 'smex-cache)
      (smex-save-to-file)))
  (run-at-time nil (* 5 60) 'smex-save-to-file-cyclicly)
#+END_SRC
* Analyze Emacs usage
#+BEGIN_SRC emacs-lisp
  (use-package keyfreq
